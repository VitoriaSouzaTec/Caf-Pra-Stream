...
export const POST = auth(async function POST(request) {
    if(!request.auth){
        return NextResponse.json({error: "Usuário Não encotrando"},
            {status: 401})

    }
    
    try { const account = await stripe.accounts.create({
        controller:{
            losses:{
    ...
    ///atualiza o banco do user logado com a conta criada do stripe
